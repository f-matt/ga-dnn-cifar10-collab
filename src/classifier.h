...
public:
	...
	virtual void classify(const T& pattern, int& pred) = 0;

	virtual void classify(T& pattern) = 0;


	Classifier(const TopologyDescriptor &descriptor,
			int output_size,
			int batch_size,
			int max_epochs,
...
protected:
	...
	void add_softmax_loss_layer() {
		...
		loss_parameters->set_type("SoftmaxWithLoss");
		loss_parameters->set_name("loss");
		loss_parameters->set_phase(Phase::TRAIN);
		loss_parameters->add_top("loss");
		loss_parameters->add_bottom("output");
		loss_parameters->add_bottom("label");

		LayerParameter *acc_parameters = parameters->add_layer();
		acc_parameters->set_type("Accuracy");
		acc_parameters->set_name("accuracy");
		acc_parameters->add_top("accuracy");
		acc_parameters->add_bottom("output");
		acc_parameters->add_bottom("label");

		LayerParameter *softmax_parameters = parameters->add_layer();
		softmax_parameters->set_type("Softmax");
		softmax_parameters->set_name("softmax");
		softmax_parameters->set_phase(Phase::TEST);
		softmax_parameters->add_top("softmax");
		softmax_parameters->add_bottom("output");

		LayerParameter *argmax_parameters = parameters->add_layer();
		argmax_parameters->set_type("ArgMax");
		argmax_parameters->set_name("argmax");
		argmax_parameters->add_top("argmax");
		argmax_parameters->set_phase(Phase::TEST);
		argmax_parameters->add_bottom("softmax");
	}
