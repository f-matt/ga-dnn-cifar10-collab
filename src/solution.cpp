...
Solution::Solution(boost::shared_ptr<Cifar10DataWrapper> &data_wrapper_ptr) :
...
Solution::Solution(boost::shared_ptr<Cifar10DataWrapper> &data_wrapper_ptr, const string& descriptor) :
				data_wrapper(data_wrapper_ptr),
				train_acc(0),
				test_acc(0),
				predicted(false) {
...
void Solution::init() {
	...
	Cifar10Classifier classifier(descriptor,
			data_wrapper,
			N_CLASSES,
			BATCH_SIZE,
			MAX_EPOCHS,
	...
	n_weights = classifier.get_n_weights();
...
void Solution::evaluate() {
	...
	classifier.train();
	train_acc = classifier.get_train_acc();
	test_acc  = classifier.get_test_acc();
	n_weights = classifier.get_n_weights();
...
float Solution::get_train_acc() {
	return train_acc;
}


float Solution::get_test_acc() {
	return test_acc;
}


void Solution::set_train_acc(float m_train_acc) {
	train_acc = m_train_acc;
}


void Solution::set_test_acc(float m_test_acc) {
	test_acc = m_test_acc;
}


bool Solution::operator< (const Solution &other) const {
	return test_acc < other.test_acc;
}
...
Solution& Solution::operator= (const Solution& other) {
	...
	this->train_acc = other.train_acc;
	this->test_acc = other.test_acc;
